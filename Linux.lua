...
Linux.UseAcrylic = false
Linux.AcrylicModel = nil
Linux.DepthOfFieldEffect = nil
Linux.BlurFolder = nil
...
function Linux.createAcrylic3DPart()
    local Part = Linux.Instance("Part", {
        Name = "LinuxAcrylicBlur",
        ...
        CastShadow = false,
        Transparency = 0.98,
        Parent = Linux.BlurFolder
    })
    
    Linux.Instance("SpecialMesh", {
        Parent = Part,
...
function Linux.updateAcrylic3DPart(uiFrame)
    if not Linux.UseAcrylic or not Linux.AcrylicModel or not uiFrame then
        return
    end
    ...
    local size = uiFrame.AbsoluteSize - Vector2.new(offset, offset)
    local position = uiFrame.AbsolutePosition + Vector2.new(offset / 2, offset / 2)
    ...
    Linux.AcrylicModel.CFrame = CFrame.fromMatrix(
        (topLeft3D + bottomRight3D) / 2,
        camera.XVector,
        camera.YVector,
        camera.ZVector
    )
    Linux.AcrylicModel.Mesh.Scale = Vector3.new(width, height, 0)
end

function Linux.enableDepthOfField()
    if Linux.DepthOfFieldEffect then
        return
    end
    
    Linux.DepthOfFieldEffect = Linux.Instance("DepthOfFieldEffect", {
        Parent = Lighting,
        FarIntensity = 0,
        InFocusRadius = 0.1,
        NearIntensity = 1
...
function Linux.disableDepthOfField()
    if Linux.DepthOfFieldEffect then
        Linux.DepthOfFieldEffect:Destroy()
        Linux.DepthOfFieldEffect = nil
    end
end
...
function Linux.Destroy()
    ...
    if Linux.UseAcrylic then
        Linux.disableDepthOfField()
        if Linux.AcrylicModel then
            Linux.AcrylicModel:Destroy()
            Linux.AcrylicModel = nil
        end
        if Linux.BlurFolder then
            Linux.BlurFolder:Destroy()
            Linux.BlurFolder = nil
        end
    end
...
function Linux.Create(config)
    ...
    if config.AcrylicEnabled ~= nil then
        Linux.UseAcrylic = config.AcrylicEnabled
    end
    ...
    if Linux.UseAcrylic then
        Linux.BlurFolder = Linux.Instance("Folder", {
            Name = "LinuxAcrylicBlur",
            Parent = Workspace.CurrentCamera
        })
        Linux.AcrylicModel = Linux.createAcrylic3DPart()
        Linux.enableDepthOfField()
    end
            ...
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                if isDragging then
                    ...
                    if not hasMoved then
                        ...
                        if Linux.UseAcrylic and Linux.AcrylicModel then
                            Linux.AcrylicModel.Transparency = LinuxUI.Enabled and 0.98 or 1
                        end
    ...
    if Linux.UseAcrylic then
        local AcrylicBackground = Linux.Instance("Frame", {
            Parent = Main,
        ...
        Linux.Instance("UICorner", {
            Parent = AcrylicBackground,
            CornerRadius = UDim.new(0, 12)
        })
        
        local AcrylicGradient = Linux.Instance("Frame", {
            Parent = Main,
        ...
        local AcrylicBorder = Linux.Instance("Frame", {
            Parent = Main,
            BackgroundTransparency = 1,
            Size = UDim2.fromScale(1, 1),
            ZIndex = 0
        ...
        globalConnectionManager:addConnection(Main:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
            Linux.updateAcrylic3DPart(Main)
        end))
        
        globalConnectionManager:addConnection(Main:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
            Linux.updateAcrylic3DPart(Main)
        end))
        
        globalConnectionManager:addConnection(LinuxUI:GetPropertyChangedSignal("Enabled"):Connect(function()
            if Linux.AcrylicModel then
                Linux.AcrylicModel.Transparency = LinuxUI.Enabled and 0.98 or 1
            end
        end))
    ...
    globalConnectionManager:addConnection(MinimizeButton.MouseButton1Click:Connect(function()
        ...
        if isMinimized then
            ...
            if Linux.UseAcrylic and Linux.AcrylicModel then
                Linux.AcrylicModel.Transparency = 1
            end
        else
            ...
            if Linux.UseAcrylic and Linux.AcrylicModel then
                Linux.AcrylicModel.Transparency = 0.98
                Linux.updateAcrylic3DPart(Main)
            end
        end
    ...
    function LinuxLib.Tab(config)
        ...
        if SectionText and SectionText ~= "" then
            ...
            local SectionLabel = Linux.Instance("TextLabel", {
                Parent = SectionFrame,
                BackgroundTransparency = 1,
                Size = UDim2.new(1, CollapseEnable and -22 or -5, 1, 0),
            ...
            Linux.TabSections[currentSectionIndex] = {
            ...
            local function toggleSectionCollapse(isInitialization)
                if not CollapseEnable then return end
                
                local sectionData = Linux.TabSections[currentSectionIndex]
                if not isInitialization then
                    sectionData.IsCollapsed = not sectionData.IsCollapsed
                end
                
                if SectionIcon then
                    local targetRotation = sectionData.IsCollapsed and 0 or 90
                    if not isInitialization then
                        ...
                        SectionIcon.Rotation = targetRotation
                    end
                end
                
                for _, tabData in pairs(sectionData.ControlledTabs) do
                    if tabData.Button then
                        tabData.Button.Visible = not sectionData.IsCollapsed
                    end
                end
            end
            
            globalConnectionManager:addConnection(SectionButton.MouseButton1Click:Connect(function()
                toggleSectionCollapse(false)
            end))
            
            task.defer(function()
                task.wait(0.2)
                if CollapseEnable then
                    toggleSectionCollapse(true)
                end
            end)
        end
        ...
        if currentSectionIndex and Linux.TabSections[currentSectionIndex] then
            table.insert(Linux.TabSections[currentSectionIndex].ControlledTabs, {
                Button = TabBtn,
                TabIndex = tabIndex
            })
        end
        ...
        function TabElements.Button(config)
                ...
                task.defer(function()
                    ...
                    for _, elementData in pairs(currentTab.Elements) do
                        ...
                        if buttonFound and elementsToHide < CollapseCount then
                            ...
                            originalSizes[elementData.Instance] = elementData.Instance.Size
            ...
            local function toggleCollapseElements()
                ...
                for i, element in pairs(targetElements) do
                    if element and element.Parent then
                        if IsCollapsed then
                            local sizeTween = TweenService:Create(element, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                                Size = UDim2.new(1, 0, 0, 0)
                            })
                            globalConnectionManager:addTween(sizeTween)
                            sizeTween:Play()
                            ...
                            local sizeTween = TweenService:Create(element, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                                Size = originalSizes[element] or UDim2.new(1, 0, 0, 42)
                            })
                            globalConnectionManager:addTween(sizeTween)
                            sizeTween:Play()
                        end
